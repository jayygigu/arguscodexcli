  ...
  if (!candidatures || candidatures.length === 0) {
    return (
      <div className="border border-border rounded-lg p-6 bg-card">
        <h2 className="text-lg font-montserrat font-semibold text-foreground mb-4">Candidatures à traiter</h2>
        <div className="text-center py-8">
          <div className="w-12 h-12 rounded-full bg-muted flex items-center justify-center mx-auto mb-3">
            <User className="w-6 h-6 text-muted-foreground" />
          </div>
          <p className="text-muted-foreground font-urbanist">Aucune candidature en attente</p>
    ...
    <div className="border border-border rounded-lg bg-card overflow-hidden">
      <div className="flex items-center justify-between p-4 border-b border-border">
        <div className="flex items-center gap-3">
          <h2 className="text-lg font-montserrat font-semibold text-foreground">Candidatures à traiter</h2>
          <span className="bg-primary text-primary-foreground text-xs font-medium px-2 py-0.5 rounded-full">
            {candidatures.length}
          </span>
        </div>
        <Link href="/agence/candidatures">
          <Button variant="ghost" size="sm" className="text-primary font-urbanist">
            Tout voir
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-urbanist font-medium text-foreground truncate">
                  ...
                  <div className="flex items-center gap-4 text-sm text-muted-foreground font-urbanist">
                    ...
                    {candidature.investigator_completed_mandates > 0 && (
                      <span className="text-xs">
                        {candidature.investigator_completed_mandates} mandat
                        {candidature.investigator_completed_mandates > 1 ? "s" : ""} complété
                        {candidature.investigator_completed_mandates > 1 ? "s" : ""}
                      </span>
                    )}
                ...
                <Button size="sm" className="bg-primary hover:bg-primary/90 font-urbanist shrink-0">
                  Examiner
                </Button>
              </div>
            </div>
      ...
      {candidatures.length > 5 && (
        <div className="p-3 bg-muted/30 border-t border-border">
          <Link href="/agence/candidatures">
            <Button variant="ghost" className="w-full font-urbanist text-primary">
